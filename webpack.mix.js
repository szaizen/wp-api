...
const basePath = (process.env.MIX_BASE_PATH || "").replace(/\/$/, "");
...
mix
    ...
    plugins: [
      new SVGSpritemapPlugin(`${srcRelativePath}/assets/svg/sprite/*.svg`, {
        output: {
          ...
          svgo: {
            plugins: [{ addClassesToSVGElement: { className: "svg-sprite" } }]
          },
          svg4everybody: true
        }
      })
  ...
  .browserSync({
    ...
    files: [`${distRelativePath}/assets/**/*`, `${distRelativePath}/**/*.html`],
    https:
    ...
    distRelativePath,
    {
      mix: (filePath = "") =>
        process.env.NODE_ENV === "production"
          ? "/zenchan/wp-api" + basePath + filePath + "?id=" + Date.now()
          : basePath + filePath,
